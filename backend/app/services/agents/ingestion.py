...
def build_ingestion_graph():
    async def parse_document(state: IngestionState) -> IngestionState:
        ...
        is_gibberish = total_count == 0 or (total_count > 50 and alpha_count / total_count < 0.5)
        
        # Trigger OCR fallback ONLY if it's a PDF. JSON/Text should not be OCR'd.
        if is_gibberish and mime == "application/pdf":
            logger.warning(
    ...
    async def extract_metadata(state: IngestionState) -> IngestionState:
        ...
        try:
            ...
            if missing_deterministic:
                ...
                logger.info("Document %s: using LLM to generate missing metadata (title/author)", state.document_id)
                llm = LiteLLMClient(model=get_settings().ollama_model_reasoner)
                metadata_prompt = (
    ...
    async def embed_chunks(state: IngestionState) -> IngestionState:
        ...
        embedded_chunks = await embed_chunks_enriched(
            chunks,
            embed_model=state.embed_model or get_settings().ollama_model_embed,

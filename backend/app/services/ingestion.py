...
def chunk_markdown(text: str, child_words: int = 80) -> list[tuple[str, str, str | None]]:
    lines = text.split("\n")
    sections = []
    current_headings = []
    ...
    for sec in sections:
        ...
        section_heading = sec["heading_path"] or None
        ...
        for child in children:
            if child.strip():
                ...
                result.append((child_with_context, parent_content, section_heading))
...
def chunk_text_hierarchical(
    ...
    if suffix in {"xlsx", "csv"}:
        ...
        for i in range(0, len(data_rows), chunk_size):
            ...
            result.append((child_text, parent_text, None))
        return result
    ...
    for parent_text in parents:
        ...
        for child in children:
            if child and child not in seen_children:
                ...
                result.append((child, parent_text, None))
...
async def embed_chunks(
    ...
    for index, (child, parent, _section_heading) in enumerate(chunks):
...
def _chunk_meets_quality(child: str) -> bool:
    """Return False if chunk is too short or too noisy for Q-gen enrichment."""
    settings = get_settings()
    if len(child) < settings.chunk_quality_min_chars:
        return False
    if not child.strip():
        return False
    alnum_count = sum(1 for ch in child if ch.isalnum())
    total = len(child.strip())
    if total > 0 and alnum_count / total < settings.chunk_quality_min_alphanumeric_ratio:
        return False
    return True
...
async def embed_chunks_enriched(
    ...
    async def process_single_chunk(
        index: int, child: str, parent: str, section_heading: str | None,
    ):
        ...
        # Only run Q-gen enrichment if chunk meets quality threshold
        if _chunk_meets_quality(child):
            async with semaphore:
                logger.info("Chunk %d: running Q-gen enrichment via LLM", index)
                enrichment_prompt = (
    ...
    tasks = [
        process_single_chunk(i, child, parent, sh)
        for i, (child, parent, sh) in enumerate(chunks)
    ]

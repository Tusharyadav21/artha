...
def build_rag_graph(db: AsyncSession, llm_client: BaseLLMClient):
    ...
    async def retrieve(state: RagState) -> RagState:
        ...
        return state.model_copy(update={"sources": sources})
    ...
    async def rerank(state: RagState) -> RagState:
        ...
        return state.model_copy(update={"sources": new_sources})

    async def quality_gate(state: RagState) -> RagState:
        if not state.sources:
            return state.model_copy(update={"low_confidence": True})
        best_score = state.sources[0].score
        return state.model_copy(
            update={"low_confidence": best_score < settings.relevance_threshold}
        )

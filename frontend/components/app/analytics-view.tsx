        ...
        <ScrollArea className="flex-1 px-8 py-6">
          <div className="max-w-3xl mx-auto space-y-6 pb-6 pr-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Queries Answered", value: totalQueries, subtitle: "+12.4% vs last week", icon: ActivityIcon, color: "text-emerald-500" },
                { label: "Avg LLM Latency", value: "1.84s", subtitle: "Ollama generation time", icon: ClockIcon, color: "text-emerald-500" },
                { label: "Vector Success", value: "99.8%", subtitle: " pgvector matching accuracy", icon: CheckCircle2Icon, color: "text-emerald-500" },
                { label: "Knowledge Nodes", value: activeDocs, subtitle: "active project sources", icon: LayersIcon, color: "text-emerald-500" },
              ].map((card, idx) => {
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between pb-1">
                <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                  <DatabaseIcon className="size-3.5 text-zinc-500" />
              ...
              <div className="space-y-3">
                {ragDensityData.map((doc, idx) => (
                  <div
                    key={idx}
                    className="p-4 rounded-xl border   hover:border-zinc-800 transition duration-150 space-y-3.5"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 min-w-0">
                        <FileTextIcon className="size-3.5 text-emerald-400 shrink-0" />
                        <span className="text-xs font-semibold text-white truncate max-w-[200px]" title={doc.filename}>
                          {doc.filename}
                        </span>
                      </div>
                      <Badge className="bg-emerald-600/10 border border-emerald-500/20 text-emerald-400 text-[9px] font-bold uppercase h-5 px-2">
            ...
            <div className="space-y-3 pt-2">
              <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                <ServerIcon className="size-3.5 text-zinc-500" />
                Vector Infrastructure Status
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl border   leading-normal flex items-start gap-3">
                  <div className="size-8 rounded-lg bg-emerald-950/20 text-emerald-400 border border-emerald-800/40 flex items-center justify-center shrink-0">
                    <DatabaseIcon className="size-4" />
                  </div>
                  <div>
                    <h4 className="text-xs font-bold text-white">pgvector Database</h4>
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      HNSW vector indexing optimized. L2-cosine similarity index status: <span className="text-emerald-400 font-semibold">Active</span>.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl border   leading-normal flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-white">Embedding Node</h4>
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      Ollama text-embeddings active. Target dimension schema: <span className="text-emerald-400 font-semibold">1536-dim</span>.
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="w-[360px] flex flex-col  h-full shrink-0">
        ...
        <header className="h-14 border-b  flex items-center justify-between px-5 shrink-0">
          <div className="flex items-center gap-1.5 select-none">
            ...
            <h2 className="text-[11px] font-bold tracking-wide uppercase text-white">
              System Health & Trace
            </h2>
          </div>
        ...
        <div className="flex-1 p-5 overflow-hidden flex flex-col min-h-0 select-none">
          <div className="flex items-center gap-1.5 shrink-0 mb-3 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
            <ActivityIcon className="size-3.5 text-zinc-500" />
            Active Pipeline Trace Logs
          </div>

          {/* Scrolling log block */}
          <ScrollArea className="flex-1 rounded-xl p-3.5">
            <div className="font-mono text-[9px] leading-relaxed space-y-3 pr-1.5 text-zinc-400">
              {logs.map((log, idx) => (
                <div key={idx} className="flex gap-2 items-start transition duration-150">
                  ...
                  <span className={cn(
                    ...
                    log.source === "VECTOR" ? "bg-emerald-950/20 text-emerald-400 border-emerald-800/40" :
                      log.source === "LLM" ? "bg-blue-950/20 text-blue-400 border-blue-800/40" :
                        log.source === "RAG" ? "bg-purple-950/20 text-purple-400 border-purple-800/40" :
                          "bg-zinc-800/30 text-zinc-400 border-zinc-700/50"
                  )}>
                    {log.source}
                  </span>
              ...
              {[
                ...
                <div key={idx} className="space-y-1">
                  ...
                  <div className="h-1  rounded-full overflow-hidden border ">
                    <div className="h-full bg-emerald-600 rounded-full" style={{ width: `${index.progress}%` }} />
                  </div>
                </div>
              ))}
            </div>
